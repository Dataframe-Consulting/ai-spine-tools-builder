...
class ReleaseManager {
  ...
  generateChangelog(analysis, newVersion, options = {}) {
    ...
    if (analysis.breaking.length > 0) {
      ...
      for (const commit of analysis.breaking) {
        ...
        const commitLink = includeCommitLinks
  ...
  validateSemanticVersioning(currentVersion, newVersion, analysis) {
    ...
    if (!currentSemver || !newSemver) {
      throw new Error(
        `Invalid version format. Current: ${currentVersion}, New: ${newVersion}`
      );
    }
    ...
    if (actualLevel < expectedLevel) {
      this.log(
        `⚠️ Warning: Version bump '${actualBump}' may be insufficient for the changes made`,
        'warning'
      );
      this.log(
        `   Expected: '${expectedBump}' based on commit analysis`,
        'warning'
      );
      ...
      this.log(
        `ℹ️ Version bump '${actualBump}' is higher than suggested '${expectedBump}'`,
        'info'
      );
      this.log(
        '   This is acceptable for ensuring proper semantic versioning',
        'info'
      );
    }
    ...
    if (analysis.breaking.length > 0 && actualBump !== 'major') {
      this.log(
        `⚠️ Warning: Breaking changes detected but version bump is '${actualBump}', not 'major'`,
        'warning'
      );
    }
  ...
  async bumpVersion(bumpType, customVersion = null, analysis = null) {
    ...
    if (analysis) {
      this.validateSemanticVersioning(
        this.currentVersion,
        newVersion,
        analysis
      );
    ...
    this.log(
      `Bumping version from ${this.currentVersion} to ${newVersion}`,
      'info'
    );
    ...
    for (const pkg of this.packages) {
      try {
        ...
        if (!skipVersionCheck && !dryRun) {
          try {
            ...
            if (existingVersion === packageVersion) {
              this.log(
                `Version ${packageVersion} already exists for ${packageName}, skipping`,
                'warning'
              );
              publishResults.push({
                package: packageName,
            ...
            this.log(
              `Could not check existing version for ${packageName}: ${error.message}`,
              'warning'
            );
          }
        }
        ...
        try {
          ...
          this.log(
            `Build failed for ${packageName}: ${error.message}`,
            'error'
          );
          throw error;
        }
        ...
        if (!dryRun) {
          try {
            ...
            this.log(
              `Package verification passed for ${packageName}`,
              'success'
            );
          } catch (error) {
            this.log(
              `Package verification failed for ${packageName}: ${error.message}`,
              'error'
            );
            throw error;
          }
        ...
        if (dryRun) {
          this.log(
            `[DRY RUN] Would publish ${packageName}@${packageVersion}`,
            'debug'
          );
          publishResults.push({
            package: packageName,
          ...
          try {
            ...
            this.log(
              `Failed to publish ${packageName}: ${error.message}`,
              'error'
            );
            publishResults.push({
              package: packageName,
        ...
        this.log(
          `Error processing package ${pkg.name}: ${error.message}`,
          'error'
        );
        publishResults.push({
          package: pkg.name,
  ...
  async verifyPublishedPackages(version, options = {}) {
    ...
    for (const pkg of this.packages) {
      ...
      while (
        ...
        try {
          ...
          if (availableVersion === version) {
            verified = true;
            this.log(
              `✅ ${packageName}@${version} is available on npm`,
              'success'
            );
            ...
            if (attempts < maxAttempts) {
              this.log(
                `Waiting for ${packageName}@${version} to propagate... (attempt ${attempts}/${maxAttempts})`,
                'debug'
              );
          ...
          if (attempts >= maxAttempts) {
            this.log(
              `Failed to verify ${packageName}@${version}: ${error.message}`,
              'error'
            );
            verificationResults.push({
              package: packageName,
      ...
      if (!verified && attempts >= maxAttempts) {
        this.log(
          `❌ ${packageName}@${version} verification timed out`,
          'error'
        );
        verificationResults.push({
          package: packageName,
  ...
  async rollbackRelease(version, options = {}) {
    ...
    if (!force) {
      this.log(
        'Rollback is a destructive operation. Use --force to confirm.',
        'error'
      );
      ...
      if (unpublish) {
        ...
        for (const pkg of this.packages) {
          try {
            ...
            this.log(
              `Failed to unpublish ${pkg.packageJson.name}@${version}: ${error.message}`,
              'error'
            );
            rollbackResults.push({
              action: 'unpublish',
  ...
  async runPreReleaseChecks() {
    ...
    if (!allPassed) {
      throw new Error(
        'Pre-release checks failed. Please fix the issues and try again.'
      );
    }
  ...
  async performRelease(options = {}) {
    ...
    try {
      ...
      if (!skipPublish && !this.dryRun) {
        ...
        try {
          ...
          if (!skipVerify) {
            ...
            const verificationResults = await this.verifyPublishedPackages(
              newVersion,
              {
            ...
            if (!allVerified) {
              this.log(
                '⚠️ Some packages could not be verified, but release continues',
                'warning'
              );
            }
          }
          ...
          this.log(
            'Release created but packages not published. You can publish manually later.',
            'warning'
          );
      ...
      if (skipPublish || this.dryRun) {
        ...
        this.log(
          '2. Publish packages: node scripts/release-manager.js publish',
          'info'
        );
      } else if (releaseResult.publishError) {
        this.log('Manual steps required:', 'info');
        this.log(
          '1. Fix publishing issue and retry: node scripts/release-manager.js publish',
          'info'
        );
        this.log(
          '2. Or rollback: node scripts/release-manager.js rollback ' +
            newVersion +
            ' --force',
          'info'
        );
        ...
        this.packages.forEach(pkg => {
          this.log(
            `  npm install ${pkg.packageJson.name}@${newVersion}`,
            'info'
          );
        });
      }
  ...
  printReleasePreview(bumpType = null) {
    ...
    console.log(
      `${colors.bold}${colors.cyan}📋 Release Preview${colors.reset}`
    );
  ...
  try {
    switch (command) {
            ...
            result.status === 'published'
              ...
              : result.status === 'skipped'
                ...
                : result.status === 'dry_run'
                  ...
                  : '❌';
          console.log(
            `  ${status} ${result.package}@${result.version} - ${result.status}`
          );
      ...
      case 'verify':
        ...
        const verificationResults = await manager.verifyPublishedPackages(
          verifyVersion,
          {
      ...
      case 'rollback':
        ...
        if (!rollbackVersion) {
          console.error(
            `${colors.red}❌ Error: Version required for rollback${colors.reset}`
          );
          process.exit(1);
        }
        ...
        if (!rollbackOptions.force) {
          console.log(
            `${colors.yellow}⚠️ Rollback is a destructive operation. Use --force to confirm.${colors.reset}`
          );
          process.exit(1);
        }

        const rollbackResults = await manager.rollbackRelease(
          rollbackVersion,
          rollbackOptions
        );
          ...
          const status =
            result.status === 'success'
              ...
              : result.status === 'not_found'
                ...
                : '❌';
          console.log(
            `  ${status} ${result.action}${result.package ? ` (${result.package})` : ''} - ${result.status}`
          );
          if (result.error) console.log(`    Error: ${result.error}`);
        });
        break;
      ...
      default:
        console.log(
          `${colors.bold}AI Spine Tools Release Manager${colors.reset}`
        );
        console.log('');
        console.log(
        ...
        console.log(
          '  verify [version]         Verify published packages are available'
        );
        console.log(
          '  rollback <version>       Rollback a release (requires --force)'
        );
        ...
        console.log(
          '  --force                  Force destructive operations (rollback)'
        );
        console.log(
          '  --unpublish              Unpublish packages during rollback'
        );
        console.log(
          '  --timeout <ms>           Timeout for verification (default: 60000)'
        );
        console.log('');
        console.log('Examples:');
        console.log('  node scripts/release-manager.js preview minor');
        console.log(
          '  node scripts/release-manager.js release minor --dry-run'
        );
        console.log('  node scripts/release-manager.js publish --verbose');
        console.log('  node scripts/release-manager.js verify 1.2.3');
        console.log(
          '  node scripts/release-manager.js rollback 1.2.3 --force --unpublish'
        );
        break;
    }
  } catch (error) {
