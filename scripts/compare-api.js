...
class APIComparator {
  ...
  compareAPIs(baselinePath, currentPath) {
    ...
    this.log(
      `Comparing API changes from v${baseline.version} to v${current.version}...`
    );
  ...
  comparePackage(packageName, baselinePackage, currentPackage) {
    ...
    if (baselinePackage.error || currentPackage.error) {
      this.log(
        `Skipping package ${packageName} due to extraction errors`,
        'warning'
      );
      return;
    }
  ...
  compareInterfaces(packageName, baselineInterfaces, currentInterfaces) {
    const baselineMap = new Map(
      baselineInterfaces?.map(iface => [iface.name, iface]) || []
    );
    const currentMap = new Map(
      currentInterfaces?.map(iface => [iface.name, iface]) || []
    );
  ...
  compareInterfaceMembers(
    ...
    const baselineMap = new Map(
      baselineMembers?.map(member => [member.name, member]) || []
    );
    const currentMap = new Map(
      currentMembers?.map(member => [member.name, member]) || []
    );
  ...
  compareTypes(packageName, baselineTypes, currentTypes) {
    const baselineMap = new Map(
      baselineTypes?.map(type => [type.name, type]) || []
    );
    const currentMap = new Map(
      currentTypes?.map(type => [type.name, type]) || []
    );
  ...
  compareFunctions(packageName, baselineFunctions, currentFunctions) {
    const baselineMap = new Map(
      baselineFunctions?.map(fn => [fn.name, fn]) || []
    );
    const currentMap = new Map(
      currentFunctions?.map(fn => [fn.name, fn]) || []
    );
  ...
  compareClasses(packageName, baselineClasses, currentClasses) {
    const baselineMap = new Map(
      baselineClasses?.map(cls => [cls.name, cls]) || []
    );
    const currentMap = new Map(
      currentClasses?.map(cls => [cls.name, cls]) || []
    );
  ...
  compareClassMembers(packageName, className, baseline, current) {
    // Compare methods
    const baselineMethodMap = new Map(
      baseline.methods?.map(method => [method.name, method]) || []
    );
    const currentMethodMap = new Map(
      current.methods?.map(method => [method.name, method]) || []
    );
    ...
    baselineMethodMap.forEach((baselineMethod, name) => {
      if (!currentMethodMap.has(name)) {
        ...
        this.compareFunctionSignatures(
          packageName,
          `${className}.${name}`,
          baselineMethod,
          currentMethod
    ...
    // Compare properties
    const baselinePropMap = new Map(
      baseline.properties?.map(prop => [prop.name, prop]) || []
    );
    const currentPropMap = new Map(
      current.properties?.map(prop => [prop.name, prop]) || []
    );
  ...
  printResults() {
    ...
    console.log(
      `${colors.cyan}Breaking Changes: ${this.comparison.breaking.length}${colors.reset}`
    );
    console.log(
      `${colors.cyan}Additions: ${this.comparison.additions.length}${colors.reset}`
    );
    console.log(
      `${colors.cyan}Non-breaking Changes: ${this.comparison.nonBreaking.length}${colors.reset}`
    );
    console.log(
      `${colors.cyan}Recommended Version Bump: ${this.comparison.summary.recommendedVersionBump}${colors.reset}\n`
    );

    // Print breaking changes
    if (this.comparison.breaking.length > 0) {
      console.log(
        `${colors.red}${colors.bold}💥 Breaking Changes:${colors.reset}`
      );
      this.comparison.breaking.forEach(change => {
        console.log(
          `  ${colors.red}❌${colors.reset} ${change.package}: ${change.change} - ${change.item}`
        );
        if (change.details) {
    ...
    if (this.comparison.additions.length > 0 && this.verbose) {
      ...
      this.comparison.additions.forEach(change => {
        console.log(
          `  ${colors.green}✅${colors.reset} ${change.package}: ${change.change} - ${change.item}`
        );
      });
      console.log('');
    }
    ...
    if (this.comparison.breaking.length > 0) {
      console.log(
        `${colors.red}${colors.bold}❌ Breaking changes detected!${colors.reset}`
      );
      console.log(
        `${colors.yellow}Please review the changes above and consider:${colors.reset}`
      );
      console.log(
        `${colors.yellow}1. Whether these changes are intentional${colors.reset}`
      );
      console.log(
        `${colors.yellow}2. Updating the major version number${colors.reset}`
      );
      console.log(
        `${colors.yellow}3. Providing migration documentation${colors.reset}`
      );
      process.exit(1);
    } else {
      console.log(
