...
function checkPackageBundles(packageName, packagePath) {
  ...
  if (!fs.existsSync(distPath)) {
    console.log(`${colors.yellow}⚠️  No dist folder found for ${packageName}${colors.reset}`);
  ...
  for (const file of bundleFiles) {
    ...
    if (sizes) {
      ...
      const status = exceedsLimit ? `${colors.red}❌ EXCEEDS LIMIT` : `${colors.green}✅ OK`;
      
      console.log(`   ${file.padEnd(15)} ${rawSizeKB.padStart(8)}KB (${gzippedSizeKB.padStart(8)}KB gzipped) ${status}${colors.reset}`);
  ...
  if (dtsSize) {
    ...
    console.log(`   ${'index.d.ts'.padEnd(15)} ${dtsSizeKB.padStart(8)}KB ${colors.blue}(types)${colors.reset}`);
  }
...
function main() {
  ...
  for (const [packageName, result] of Object.entries(summary)) {
    ...
    console.log(`${packageName}: ${status}${colors.reset} (${totalFiles} files, ${exceedingFiles} exceeding limits)`);
  }
