    ...
    buildFieldSchema(field, fieldName, _options) {
        ...
        switch (field.type) {
            case 'string':
                ...
                break;
            case 'email':
                schema = zod_1.z.string().email(_options.customMessages?.[`${fieldName}.email`] ||
                    `${fieldName} must be a valid email address`);
                break;
            case 'url':
                schema = zod_1.z.string().url(_options.customMessages?.[`${fieldName}.url`] ||
                    `${fieldName} must be a valid URL`);
                break;
            case 'uuid':
                schema = zod_1.z.string().uuid(_options.customMessages?.[`${fieldName}.uuid`] ||
            ...
            case 'datetime':
                ...
                break;
            case 'time':
                schema = zod_1.z.string().regex(/^([01]?[0-9]|2[0-3]):[0-5][0-9]:[0-5][0-9]$/, _options.customMessages?.[`${fieldName}.time`] ||
    ...
    buildConfigFieldSchema(field, fieldName, _options) {
        ...
        if (field.validation) {
            ...
            if (validation.min !== undefined && schema instanceof zod_1.z.ZodString) {
                schema = schema.min(validation.min, validation.errorMessage || `${fieldName} must be at least ${validation.min} characters long`);
            }
            if (validation.max !== undefined && schema instanceof zod_1.z.ZodString) {
                schema = schema.max(validation.max, validation.errorMessage || `${fieldName} must be at most ${validation.max} characters long`);
            }
            if (validation.pattern && schema instanceof zod_1.z.ZodString) {
                schema = schema.regex(new RegExp(validation.pattern), validation.errorMessage || `${fieldName} does not match required pattern`);
            }
        }
        // Handle required/optional and defaults
        if (!field.required) {
...
class SchemaValidator {
    ...
    static async validateInput(input, schema) {
        ...
        if (!result.success) {
            ...
            throw new types_js_1.ValidationError(`Input validation failed: ${errorMessages.join(', ')}`, result.errors[0]?.path.join('.'), { errors: result.errors });
        }
    }
    /**
    ...
    static async validateConfig(config, schema) {
        ...
        if (!result.success) {
            ...
            const missingKeys = result.errors
                ...
                .map(e => e.path.join('.'));
            throw new types_js_1.ConfigurationError(`Configuration validation failed: ${errorMessages.join(', ')}`, missingKeys);
        }
    }
}
