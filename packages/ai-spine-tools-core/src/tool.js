...
class Tool {
  ...
  validateDefinition() {
    if (!this.definition.metadata?.name) {
      throw new types_js_1.ConfigurationError(
        'Tool metadata must include a name'
      );
    }
    if (!this.definition.metadata?.version) {
      throw new types_js_1.ConfigurationError(
        'Tool metadata must include a version'
      );
    }
    if (!this.definition.metadata?.description) {
      throw new types_js_1.ConfigurationError(
        'Tool metadata must include a description'
      );
    }
    if (!this.definition.schema) {
      throw new types_js_1.ConfigurationError(
        'Tool must include a schema definition'
      );
    }
    if (typeof this.definition.execute !== 'function') {
      throw new types_js_1.ConfigurationError(
        'Tool must include an execute function'
      );
    }
  }
  /**
    ...
    this.app.use(async (req, res, next) => {
      try {
        ...
        if (apiKey && this.validApiKeys.has(apiKey)) {
          ...
          req.headers['x-api-key-hash'] = (0, crypto_1.createHash)('sha256')
            .update(apiKey)
            .digest('hex')
            .substring(0, 8);
        }
  ...
  setupRoutes() {
    ...
    this.app.post(
      '/api/execute',
      this.asyncErrorHandler(async (req, res) => {
    ...
    this.app.get(
      '/',
      this.asyncErrorHandler(async (_req, res) => {
  ...
  sendStandardError(res, error, requestId) {
    const errorResponse = {
      request_id:
        requestId || res.locals.requestId || (0, crypto_1.randomUUID)(),
      status: 'error',
      error: {
        code: error.code,
    ...
    try {
      ...
      if (this.definition.healthCheck) {
        try {
          ...
          const customHealthDuration =
            perf_hooks_1.performance.now() - customHealthStart;
          checks.custom = {
      ...
      const healthCheckDuration =
        perf_hooks_1.performance.now() - healthCheckStart;
...
print(result)`,
      ...
      const schemaGenerationDuration =
        perf_hooks_1.performance.now() - schemaGenerationStart;
      ...
      const enhancedMetrics = {
        ...
        configuration_metrics: {
          ...
          monitoring_overhead_ms: Math.round(
            perf_hooks_1.performance.now() - metricsGenerationStart
          ),
        },
        // Metadata
        metrics_metadata: {
    ...
    if (
      ...
      for (const [key, field] of Object.entries(this.definition.schema.input)) {
        schema.properties.input_data.properties[key] =
          field_builders_js_1.DocumentationGenerator.generateOpenAPISchema(
            field
          );
        if (field.required) {
  ...
  async start(config = {}) {
    ...
    try {
      ...
      this.setState('error');
      throw new types_js_1.ExecutionError(
        `Failed to start tool server: ${error.message}`,
        error
      );
    }
  }
  ...
  async stop() {
    ...
    try {
      ...
      this.setState('error');
      throw new types_js_1.ExecutionError(
        `Failed to stop tool server: ${error.message}`,
        error
      );
    }
  }
