...
import { stringField, numberField, apiKeyField, enumField } from '../field-builders.js';
...
describe('Tool Class', () => {
  ...
  describe('Metrics and Monitoring', () => {
    ...
    it('should calculate requests per minute', async () => {
      ...
      await Promise.all([
        request(tool['app']).post('/api/execute').send({ input_data: { message: 'test1' } }),
        request(tool['app']).post('/api/execute').send({ input_data: { message: 'test2' } }),
        request(tool['app']).post('/api/execute').send({ input_data: { message: 'test3' } })
      ]);
  ...
  describe('Test Method', () => {
    ...
    it('should return error when no configuration is available', async () => {
      ...
      expect(testResult.valid).toBe(false);
      expect(testResult.errors).toContain('No configuration provided and no tool configuration set');
    });
  });
