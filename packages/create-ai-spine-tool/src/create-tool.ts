...
export async function createTool(
  ...
  if (validateTemplate) {
    await validateTemplateBeforeProcessing(options.template, options.language, context);
  }
  ...
  for (const file of files) {
    ...
    if (file.isDirectory()) {
      ...
      try {
        ...
        if (sourcePath.includes('bin/') || path.basename(sourcePath).startsWith('run-')) {
          try {
            await fs.chmod(targetPath, 0o755);
          } catch (chmodError) {
            if (process.env.AI_SPINE_VERBOSE) {
              console.log(chalk.yellow(`Warning: Could not set executable permissions on ${targetPath}`));
            }
          }
        }
...
async function validateTemplateBeforeProcessing(
  ...
  try {
    const result = await validator.validateTemplate(templateType, language, context, templatesDir);
    ...
    if (!result.isValid) {
      ...
      if (criticalErrors.length > 0) {
        console.error(chalk.red.bold('❌ Critical template validation errors found:'));
        ...
        throw new Error(`Template validation failed with ${criticalErrors.length} critical errors`);
      }
      
      // Show warnings for non-critical issues
      if (process.env.AI_SPINE_VERBOSE && result.warnings.length > 0) {
        console.log(chalk.yellow(`⚠️  Template validation warnings (${result.warnings.length}):`));
    ...
    if (process.env.AI_SPINE_VERBOSE) {
      console.warn(chalk.yellow(`Warning: Template validation failed: ${(validationError as Error).message}`));
...
async function validateProcessedContent(
  ...
  if (unresolvedTemplates) {
    errors.push(`Unresolved template variables found: ${unresolvedTemplates.join(', ')}`);
  }
  ...
  if (fileName === 'package.json') {
    await validatePackageJsonContent(content, sourcePath, context, errors, warnings);
  }
  ...
  if (warnings.length > 0 && process.env.AI_SPINE_VERBOSE) {
    console.warn(chalk.yellow(`⚠️  Content validation warnings in ${fileName}:`));
...
async function validateTypeScriptContent(
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const invalidVarPattern = /(?:const|let|var)\s+([^a-zA-Z_$][a-zA-Z0-9_$]*)\s*=/;
    ...
    if (idMatch) {
      ...
      if (reservedWords.includes(varName)) {
        errors.push(`Variable name "${varName}" is a reserved word at line ${lineNumber}`);
      }
    }

    // Check for potential syntax issues
    if (line.includes('function(') && !line.includes('function (')) {
      warnings.push(`Consider adding space after 'function' at line ${lineNumber}`);
    }
  }

  // Check for required imports
  if (content.includes('createTool') && !content.includes("from '@ai-spine/tools'")) {
...
async function validatePackageJsonContent(
  ...
  try {
    ...
    if (packageJson.name !== context.packageName) {
      errors.push(`Package name mismatch: expected "${context.packageName}", got "${packageJson.name}"`);
    }
    ...
    if (packageJson.dependencies) {
      for (const [dep, version] of Object.entries(packageJson.dependencies)) {
        if (typeof version !== 'string') {
          errors.push(`Invalid version for dependency ${dep}: must be a string`);
        } else if (!version.match(/^[\^~]?\d+\.\d+\.\d+/)) {
          warnings.push(`Unusual version format for dependency ${dep}: ${version}`);
        }
      }
    }

  } catch (parseError) {
    errors.push(`package.json is not valid JSON: ${(parseError as Error).message}`);
  }
}
 ...
 */
export function generateEnhancedTemplateContext(options: CreateToolOptions): TemplateContext & {
  ...
  return {
    ...
    hasDatabase: options.template === 'data-processing' || options.template === 'api-integration',
